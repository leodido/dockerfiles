...
```
docker run -it leodido/sphinxsearch:2.2.1 ls /usr/local/bin
...
```
cd dockerfiles/sphinxsearch\:2.2.1
docker build -t leodido/sphinxsearch:2.2.1 .
```

Otherwise you can pull this image from docker index.

```
docker pull leodido/sphinxsearch:2.2.1
```
...
```
docker run -i -t leodido/sphinxsearch:2.2.1 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d leodido/sphinxsearch:2.2.1 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d leodido/sphinxsearch:2.2.1 indexall.sh
```
...
[![Analytics](https://ga-beacon.appspot.com/UA-49657176-1/dockerfiles/sphinxsearch:2.2.1)](https://github.com/igrigorik/ga-beacon)

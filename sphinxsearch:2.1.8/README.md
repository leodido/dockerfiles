...
You can read [here](http://sphinxsearch.com/bugs/changelog_page.php?version_id=42) the official changelog.
...
docker build -t leodido/sphinxsearch:2.1.8 .
```

Otherwise you can pull this image from docker index.

```
docker pull leodido/sphinxsearch:2.1.8
```
...
```
docker run -i -t leodido/sphinxsearch:2.1.8 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d leodido/sphinxsearch:2.1.8 ./searchd.sh)
```
...
1. the [data source](http://sphinxsearch.com/docs/2.1.8/xmlpipe2.html) files (e.g. XML files structured as demanded by the Sphinx Search's xmlpipe2 driver)
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d leodido/sphinxsearch:2.1.8 ./indexall.sh
```

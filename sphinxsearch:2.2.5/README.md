...
```
docker run -it leodido/sphinxsearch:2.2.5 ls /usr/local/bin
...
docker build -t leodido/sphinxsearch:2.2.5 .
```

Otherwise you can pull this image from docker index.

```
docker pull leodido/sphinxsearch:2.2.5
```
...
```
docker run -i -t leodido/sphinxsearch:2.2.5 /bin/bash
```
...
```
SS=$(docker run -i -t -v $PWD:/usr/local/etc -p 9306 -d leodido/sphinxsearch:2.2.5 searchd.sh)
```
...
```
docker run -i -t -v $PWD:/usr/local/etc -p 127.0.0.1:9306:9306 -d leodido/sphinxsearch:2.2.5 indexall.sh
```
